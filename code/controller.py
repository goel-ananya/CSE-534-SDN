...
# include as part of the beta branch
from pox.openflow.of_json import *

# logger is created to log the events.
log = core.getLogger()


# port_map table maps port for every link between switches
# flow_table table stores next hop for packet transmission between given switches 
port_map ={}
flow_table={}
switches = set()
table={}
	...
	for conn in core.openflow.connections:
		...
		for s1 in switches:
			...
			if s1 == conn_id:
				...
				for s2 in switches:
					if s1!=s2:
						if (s1,s2) in port_map:
							out_port = port_map[(s1,s2)]
							...
							msg.actions.append(of.ofp_action_output(port = port_map[(s1,s2)]))
							conn.send(msg)
	log.debug("Links only flow config activated")          

#This method parses paths.file, which contains links that are active
#After parsing, links are stored in flow_table table. This table is used to add flows in switches later. 
def populate_flow_table():
	...
	for line in f:
		...
		flow_table[(x_list[0],x_list[1])] = x_list[2].rstrip()
...
def _handle_links (event):
	...
	for l in core.openflow_discovery.adjacency:
		...
		if (sw1,sw2) not in port_map:
			port_map[(sw1,sw2)]=l.port1
			port_map[(sw2,sw1)]=l.port2
		...
		if not packet.dst.is_multicast:
			...
			if dst_sw in switches:
				if cur_sw==dst_sw:
					...
					next_sw = flow_table[(cur_sw,dst_sw)]   
					msg = of.ofp_flow_mod()
					msg.match.dl_src = packet.src
					msg.match.dl_dst = packet.dst
					msg.hard_timeout = 300
					msg.data = event.ofp
					msg.actions.append(of.ofp_action_output(port = port_map[(cur_sw,next_sw)]))
					event.connection.send(msg)
